...
function normalizarRuta(ruta, crearCarpeta = false) {
    if (!ruta)
        return '';
...
function buscarArchivoConfig(nombreArchivo) {
    const exeDir = path.dirname(process.execPath);
    ...
    try {
        ...
        try {
            ...
            if (writeError.code === 'EBUSY' || writeError.code === 'EPERM' ||
                (writeError.message && (writeError.message.includes('being used by another process') ||
...
electron_1.ipcMain.handle('validar-ruta', async (event, ruta) => {
    try {
        if (!ruta)
            return false;
        const rutaNormalizada = normalizarRuta(ruta);
